...
Usage:
    # From a rendered site page (must live under ./output/, matching your
    # existing pipeline) — title, category/tag, and images are all pulled
    # out of the HTML automatically:
    python publish_to_substack.py output/articles/technology/agency.html

    # From a plain Markdown file (old behavior — title/subtitle read off
    # leading '#'/'##' lines):
    python publish_to_substack.py article.md

    python publish_to_substack.py ... --publish                # publish immediately instead of leaving as draft
    python publish_to_substack.py ... --title "Custom Title"
    python publish_to_substack.py ... --tag addiction --tag technology   # extra tags, added alongside the detected category

Requires:
    pip install python-substack python-dotenv beautifulsoup4

Auth (put in a .env file next to this script — see .env.example):
    PUBLICATION_URL=https://adamfistler.substack.com
    COOKIES_STRING="<the Cookie: request header from your logged-in browser session>"
    # or EMAIL=... / PASSWORD=...

HTML input handling:
    - Title comes from the page's <title> tag (text before the first '|').
    - Category comes from the second breadcrumb item (e.g. "Technology
      articles" -> tag "technology") and is added as a Substack tag
      automatically.
    - Redundant header elements (main `<h1>`, subtitle `##`, and author `##` blocks)
      are stripped out of the body content so they don't duplicate Substack's native fields.
    - At the very top of the document body, an "Original Source: [<Title>](<link>)"
      line and an "Adam Fistler Web Site: [https://www.adamfistler.com](https://www.adamfistler.com)"
      line are inserted.
    - <div class="content-img"> wrappers are converted to <figure>/
      <figcaption> before the HTML is turned into Markdown, and every
      link/image/media URL is rewritten to an absolute
      https://www.adamfistler.com/... URL so Substack can fetch them.
    - Nav/breadcrumb/footer/"Explore my Technology..." sections are
      stripped before conversion.
    - As before, a converted copy is also written to
      output/substack/<same relative path>.html and .md, so you keep a
      local record even though the .html/.md files are no longer a
      required manual hand-off step.

Markdown input handling (unchanged from before):
    A leading '# Title' line, optionally followed by a '#'/'##' subtitle
    line, is read off automatically. Override either with --title/--subtitle.
"""
...
# Where scaled image copies get written on disk (must be writable by the
# user running this script) and the public URL they're served from.
...
# Matches Markdown images: ![alt](url) or ![alt](url "caption")
IMAGE_MARKDOWN_RE = re.compile(r'!\[([^\]]*)\]\((\S+?)(?:\s+"([^"]*)")?\)')


# ---------------------------------------------------------------------------
# HTML -> Markdown conversion (title, category, images, cleanup)
# ---------------------------------------------------------------------------

def derive_output_paths(input_filename):
    """Mirror the input file's path under output/substack/, e.g.
    output/articles/technology/agency.html -> output/substack/articles/technology/agency.{html,md}
    Also returns the relative path itself (e.g. "articles/technology/agency.html"),
    used to build the public source URL for the page.
    """
...
def clean_content(soup):
    ...
    # Strip redundant top title/subtitle/author heading blocks from the body
    ...
    # Convert custom image wrappers into Substack-friendly <figure>/<figcaption>
    ...
    # Trim trailing whitespace/<br> left behind after removals
...
def block_to_markdown(element, list_level=0):
    ...
    if tag == "figure":
        ...
        if img is not None:
            ...
            if not src:
                return ""
            # Substack reads the Markdown image "title" as the caption shown under the image.
            if caption:
                return f'![{alt}]({src} "{caption}")\n\n'
            return f"![{alt}]({src})\n\n"

        # No <img> found — fall back to rendering whatever's inside normally.
        return "".join(
...
def convert_html_file(input_filename):
    """Returns (title, category, markdown_body). Also writes the .html/.md
    mirror copies under output/substack/ as a local record."""
    ...
    content, category = clean_content(soup)

    html_output = generate_html(content)
    markdown_output = generate_markdown(content)
    markdown_output = prepend_source_and_website_header(markdown_output, title, source_url)
    ...
    return title, category, markdown_output


# ---------------------------------------------------------------------------
# Optional: downscale embedded images (source images on the site are never
# touched — a resized copy is written to WEB_ROOT/SUBSTACK_IMG_SUBDIR and
# served over http(s) so Substack can fetch it, same as it fetches any
# other image URL in the post)
# ---------------------------------------------------------------------------

def scale_down_images(markdown_text, scale):
    """Find every Markdown image pointing at a remote http(s) URL, download
    it, resize the downloaded copy by `scale` (e.g. 0.5 = 50%), and save it
    under WEB_ROOT/SUBSTACK_IMG_SUBDIR/<same relative path as the source
    URL>. Rewrites the Markdown to point at the public URL for that copy
    instead of the original. Leaves already-local image paths untouched.
    ...
    def replace(match):
        ...
        if not url.startswith(("http://", "https://")):
            return match.group(0)  # local path already — leave it alone
        ...
        try:
            ...
            # Mirror the source URL's path under substack_img/, e.g.
            # https://www.adamfistler.com/images/photos/lionel.jpg
            #   -> WEB_ROOT/substack_img/images/photos/lionel.jpg
...
# ---------------------------------------------------------------------------
# Prepend Original Source & Website Header
# ---------------------------------------------------------------------------

WEBSITE_LINK_LINE = "Adam Fistler Web Site: [https://www.adamfistler.com](https://www.adamfistler.com)\n\n"

def prepend_source_and_website_header(markdown_text, title, source_url):
    """Prepends the Original Source line and Adam Fistler Web Site line at the top."""
...
# ---------------------------------------------------------------------------
# Plain-Markdown input (unchanged behavior for .md files)
# ---------------------------------------------------------------------------
...
def main():
    ...
    parser.add_argument("--subtitle", help="Override the auto-detected subtitle (Markdown input only)")
    ...
    parser.add_argument(
        ...
        "Downloads and resizes a copy for upload — never touches the source images on your site. "
        "Default: 1 (full size, no scaling).",
    )
    args = parser.parse_args()

    if not (0 < args.image_scale <= 1):
        sys.exit("--image-scale must be greater than 0 and at most 1 (e.g. 0.5 for 50%, 1 for full size).")
    ...
    if cookies_string:
        ...
        sys.exit("No credentials found. Set COOKIES_STRING (recommended), COOKIES_PATH, or EMAIL+PASSWORD in your .env file.")
    ...
    if suffix in (".html", ".htm"):
        try:
            detected_title, category, body_md = convert_html_file(str(args.input_file))
        except ValueError as error:
            sys.exit(str(error))
        detected_subtitle = None
        if category:
            tags = [category] + tags
    else:
        raw_md = args.input_file.read_text(encoding="utf-8")
        detected_title, detected_subtitle, body_md = extract_title_subtitle(raw_md)
        # Apply source & website header for plain Markdown too
